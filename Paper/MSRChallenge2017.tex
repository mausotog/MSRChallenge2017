...
than only using technical data?}
...
established  and interpretable. In particular, we use a J48 decision tree because this classification mechanism has the ability to identify the attributes that discriminate the various instances most clearly, and we are interested in analyzing what particular features can predict with higher accuracy build success.
We first build a
...
experience. If we were unable to find information about an author than we
...
of the associated project. These values were investigated to understand and be able to quantify the developers’ experience.
...
equal, a process known as downsampling. We downsampled the entire dataset being analyzed and then performed cross-validation on it. Since the cross-validation is stratified, the proportion of 50-50 is maintained through the training and testing folds. We train and present results for both
data sets individually in Sections \ref{eval}A, \ref{eval}B and \ref{eval}C.
...
correctly guessed instances occurring by chance. The precision, recall and F-measure are calculated over the successful and unsuccessful classes and then averaged (standard procedure in Weka). Precision and recall are calculated as follows: 
...
F-score is a correlation of these two values. We can see that all these measures increase when we add social data into the feature space, making the prediction more reliable. Besides this, the mean absolute error
...
information for). We found that 99.13\% (347775/350834) of the contributions 
were performed by
...
developers
with more followers are more likely to have a successful integration build. Reasons for this might be 
...
that project while our study focuses on a wide range of open source projects and
