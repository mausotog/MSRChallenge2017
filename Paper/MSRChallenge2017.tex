...
\clg{detail nitpick: ``et al.'' has a paragraph after the al. (and not the et).
  This is something I write about in the blog post you all claim to have read.
  Please fix it throughout.}
...
\paragraph{Goal}
The goal of this study is to identify and understand relationships 
...
\clg{I might recommend a paragraph at the top of this section defining at a high
  level the key terms, like ``technical elements'' versus ``social factors.''
  You do concretize them in the next section, but conceptually here they're a
  little bit UBD.}
...
\paragraph{Metric}
\clg{I think I need a little bit more detail here, at multiple levels (like, why
  a decision tree? among other questions.}
We created a decision tree based on two different sets of data. Merely 
...
\clg{Perhaps it makes sense to break this down into subsections?}

To investigate our research questions, we built a data set based on the 
attributes that consisted of both technical and social factors. For the technical factors, we collected
...
complexity and the build status as listed below:\clg{The listed terms don't mean
  anything on their own; they're clearly grabbed from code.  Please rename them
  to something descriptive (in English) and give a quick definition of each.
  Even better is if you give some insight as to why they might be meaningful in
  your theory (or not).}
\begin{itemize}
...
As a result of extracting just these particular technical attributes, the new
data set is conformed by a  big number of repeated rows, which would slow down
the process of collecting the social data for each of these rows, therefore, at
this point we removed the repeated rows and continued to collect the social
attributes from the developers who pushed the commits.\clg{I'm afraid this
  sentence/paragraph isn't parsing for me.  Please rephrase, clarify, expand,
  and add precision.  Also break into multiple sentences, because it's really
  really long, which doesn't help.  To start, I think the word ``conformed'' is
  not what you mean.}

To collect the social data, we created a web scraper to extract the social data
...
\textit{username} from a commit page. If the scrapper\clg{you mean ``scraper''
  not ``scrapper''; I fixed it once but you should fix it everywhere else.} was able to find username, the
scraper went to the user's profile information
(\textit{https://github.com/\textbf{UserName}}). 

The scrapper then extracted the following items from the user's
profile.\clg{slightly better than the above for technical factors because you do
  define them in the text below but I think integrating into the itemized list
  would be better.  Also, what does each of these mean?}
\begin{itemize}
\item Repositories
\item Stars
\item Followers
\item Following
\item Contributions in the last year
\end{itemize}
...
We also added one item from the original\clg{...what?} data set to the social factors:
...
factors along with the build status.\clg{This is not a complete
  sentence.}\clg{also: why do you build two data sets?}
...
significantly in the last three years.\clg{...did you spot check?}

After building the data set, we converted the\clg{which?} various Travis build statuses to
successful (passed in the\clg{which?} data set) or not successful (errored, failed, canceled
and started in the data set).  This still left a big discrepancy in the two
categories, with about a 3:1 successful to not successful ratio.\clg{I think I'm
  having ah ard time here because it's not clear which dataset we're referring
  to, but I don't understand the previous sentence, and thus don't understand
  the next sentence.} To avoid having
...
equaled the number of not successful builds (downsampling process).\clg{How?}  We used both the
downsampled and non-downsampled data sets in our results.\clg{Why?}
...
\\
\center
  \caption{ Second and third columns: Comparison of performance when running 10
    fold cross validation\clg{We've discussed this phrasing surrounding 10-fold cross
      validation.  Please fix.  What is the metric here?  How do I interpret
      these numbers?} of two decision trees. The first column after
    the first double bar uses only technical attributes to predict the Travis
    Job status, the second column augments the data set with social
    attributes. Fourth and fifth columns: \clg{this sentence is not a
      sentence/is a bit awkward.} The same intuition as the previous
    two, with the difference that the data set was downsampled to provide the
    same number of successful and unsuccessful instances.\clg{...and what is the
    takeaway from this table?} \clg{Small thing: labels should be inside the
    caption otherwise the numbers sometimes get confused.  It's a stupid latex thing.}  \label{resultsTable}} 
\end{table*}
...
\clg{I killed the separation between ``evaluation'' and ``results'' because I
  don't know why it existed in the first place.}
\clg{I also wouldn't mind an enumeration of research questions; the stuff in
  design (section 2) is high level and it's not obvious where you're going to go
  at this juncture.}
...
set. \clg{This sentence migth be better place above, when you first discuss subsampling.}
...
81.10\%, about a 4\% increase.\clg{It's not clear from this text that you're
  talking about the unsampled data.}
...
\clg{...upshot? Also: there are various numbers in the Table that aren't
  discussed or explained either in the caption or in these results, like, what's
  a Kappa statistic, and what do we learn from them?}

\subsection{Build success by core team member}

For the build success investigation,\clg{Is this a different investigation than
  the previous subsection?}  we classified the build success rate for
...
passed.  Non-core team members triggered 10.61\% of the builds in the\clg{which?} data set
...
as the ones studied in this paper are important\clg{...maybe a strong word?} predictors when
...
\clg{per my email: fix the bibliography. Now.} 
