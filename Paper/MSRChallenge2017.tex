...
to contribute, the number of commits and pull requests keep increasing.
As the number of commits and pull requests increase, it becomes more difficult
to only accept quality contributions.
...
However, with continuous integration, we can
...
in the decision~\cite{Scacchi07}.  Yet, as social coding sites continue to get
more users and specific projects become more popular, projects receive more
...
requests which must be approved by a core team member~\cite{gousios14}. Another
contribution mechanism these projects use is continuous
integration, which notifies the team members if a change caused a test 
failure~\cite{Meyer14}.
Yet, these methods can become a bottleneck.
To avoid a possible bottleneck, it would be beneficial to prioritize certain
contributions.  This prioritize can be achieved with predictors of the
contribution's quality.


Past projects have shown that both technical factors, which describe the commit
and the project receiving the commit, and social factors, which describe the
developer making the commit, have been found useful when predicting a
contribution's quality~\cite{tsay14icse,tsay14fse}.
...
We performed this analysis with a large corpus of open source projects
which incorporated continuous integration in their development practice. 
When a contribution to these projects pass integration tests, 
it shows that the project developers
...
While past studies have analyzed pull request
success~\cite{tsay14icse,tsay14fse}, we are interested in analyzing the quality 
of a contribution 
at a lower granulairity level: individual commits. \todo{Zack question for Mau:
did we really do the investigation at the individual commit level?} 
We analyzed commits in the 
context of continuous integration, using the technical factors found in the 
some of the social attributes provided in the challenge
...
This study investigates how well different technical and social factors predict
commit quality.  This information will help core team members prioritize project
contributions.  It will also provide a further understand of quality enforcement
...
pull request and the integrators is a key factor towards acceptance of a pull 
...
\textbf{Technical Factors:} We collected these factors from the 
...
\item Source churn: lines of code changed by this commit
\item Files added: files added by this commit
\item Files modified: files modified by this commit
\item Tests added: tests added by this commit
\item Tests deleted: tests deleted by this commit
\item Source files: files in project at the time of the commit
\item Source loc: lines of source code in the project at
...
This factor indicates if the commit's developer is a core member 
of that project.
...
factors. We performed this split to analyze how well technical factors alone,
...
and consists of commits from multiple years ago. The social data we 
...
a perfect solution , we believe that these numbers are unlikely to have changed
...
\todo{Zack: Mau, check this paragraph}
Since we are interested in the success of the job status of each commit, we 
...
the commit contained both passing and failing build statuses, we removed the
commit from the data set.
\todo{confirm that this number doesn't change}
At this point we had a 3:1 successful to 
...
The high ratio of successful to not successful builds cause the classifier to 
favor misclassifying not successful builds over successful builds, because a 1\%
...
successful builds equaled the number of not successful builds.  
We build this data set by copying over all not successful builds and randomly
selecting a sub-sample of the successful builds until the two build status were
equal, known as downsampling.  We used 
...
\end{table*}
To give an address to main research question: ``Do social attributes, gathered 
from the GitHub profiles of developers,
...
compared to a base line rate of guessing the build passed in all cases.
