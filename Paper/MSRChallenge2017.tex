...
complexity and the build status as listed below:
\begin{itemize}
...
\\
\center
  \caption{ Second and third columns: Comparison of performance when running 10 fold cross validation of two decision trees. The first column after the first double bar uses only technical attributes to predict the Travis Job status, the second column augments the data set with social attributes. Fourth and fifth columns: The same intuition as the previous two, with the difference that the data set was downsampled to provide the same number of successful and unsuccessful instances }
  \label{resultsTable}
\end{table*}

As a result of extracting just these particular technical attributes, the new data set is conformed by a  big number of repeated rows, which would slow down the process of collecting the social data for each of these rows, therefore, at this point we removed the repeated rows and continued to collect the social attributes from the developers who pushed the commits.
...
set provided by the MSR Challenge~\cite{msr17challenge} to locate the web page of the commit using the syntax
below. 
\newline \newline
...
equaled the number of not successful builds (downsampling process).  We used both the
...
\section{Discussion}
We have found that social attributes which proxy experience produce a 
...
as the ones studied in this paper are important predictors when
