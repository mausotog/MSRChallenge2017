...
has shown that both technical factors, which describe the commit
...
contribution's quality~\cite{tsay14icse,tsay14fse}.\todo{One or two more
  sentences, either here or earlier in the paragraph (between the first and
  second sentence), about why social factors might be interesting to study, or informative.}
...
of a contribution in the context of continuous integration. \todo{What's the
  difference, and why might it matter? Why is CI interesting or different as
  compared to pull requests?(one sentence or so).} Thus, we propose to analyze the
success every time a developer pushes a contribution to be built. 
...
contribution quality.\todo{One sentence summary of results}  This information can help core team members prioritize project
...
source code being modified by the developer initiating a pull request 
and the acceptance rate of these changes is highly correlated.\todo{The
  preceding sentence is not grammatical.  Please fix.} Also existence of 
tests and overall code quality~\cite{gousios15} are attributes that have a 
strong relationship with the acceptance rate of a pull request.\todo{the
  previous sentence is not a sentence.} Bettenburgh et 
...
success when integrated to\todo{wrong word} the automatic build process. Yu et al.~\cite{yu16} 
...
continuous integration build.\todo{This paragraph doesn't really belong in the
  related work; I might move it above, near where I have a todo asking for a
  summary of results.}
...
It is also worth noticing that in both cases the Kappa statistic increases when adding social data (100\% and 85\% respectively) which represents a more robust measure than Correctly classified instances, since it takes into account the correctly guessed instances occurring by chance, and we may also notice that the Mean absolute error decreases, giving stronger confidence to our claims when considering eventual outcomes.
